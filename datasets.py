...
class VITONDataset(data.Dataset):
    def __init__(self, opt):
        ...
        try:
            img_names = []
            c_names = []
            dataset_list_path = osp.join(opt.dataset_dir, opt.dataset_list)
            logger.info(f"Loading dataset list from: {dataset_list_path}")
                ...
                for line in f.readlines():
                    try:
                        ...
                        c_names.append(c_name)
                            ...
                            continue
            if not img_names or not c_names:
                    raise ValueError("No valid image pairs found in dataset list")
    ...
    def get_parse_agnostic(self, parse, pose_data):
        try:
            ...
            if pose_data.shape[0] < 33:  # MediaPipe has 33 landmarks
                raise ValueError(f"Invalid pose data shape: {pose_data.shape}")


            # MediaPipe landmark indices for arms
            # Left arm: shoulder (11) -> elbow (13) -> wrist (15)
            # Right arm: shoulder (12) -> elbow (14) -> wrist (16)
            for parse_id, pose_ids in [(14, [11, 13, 15]), (15, [12, 14, 16])]:
                ...
                for i in pose_ids[1:]:
                    ...
                    if not (0 <= pose_data[i_prev, 0] < self.load_width and 
                            0 <= pose_data[i_prev, 1] < self.load_height and
                            0 <= pose_data[i, 0] < self.load_width and 
                            0 <= pose_data[i, 1] < self.load_height):
                            continue
            ...
            return agnostic
                ...
                logger.error(f"Error in get_parse_agnostic: {str(e)}")
                raise
    ...
    def get_img_agnostic(self, img, parse, pose_data):
        try:
            ...
            length_a = np.linalg.norm(pose_data[11] - pose_data[13]) # Left arm
            length_b = np.linalg.norm(pose_data[12] - pose_data[14]) # Right arm
            point = (pose_data[12] + pose_data[14]) / 2
            pose_data[12] = point + (pose_data[12] - point) / length_b * length_a
            pose_data[14] = point + (pose_data[14] - point) / length_b * length_a

            # mask arms
            agnostic_draw.line([tuple(pose_data[i]) for i in [11, 13]], 'gray', width=r*10)
            for i in [11, 13]:
                pointx, pointy = pose_data[i]
                agnostic_draw.ellipse((pointx-r*5, pointy-r*5, pointx+r*5, pointy+r*5), 'gray', 'gray')
            for i in [13, 15, 14, 16]:
                if (pose_data[i - 2, 0] == 0.0 and pose_data[i - 2, 1] == 0.0) or (pose_data[i, 0] == 0.0 and pose_data[i, 1] == 0.0):
                    continue
            ...
            agnostic_draw.line([tuple(pose_data[i]) for i in [11, 23]], 'gray', width=r*6) # Left torso
            agnostic_draw.line([tuple(pose_data[i]) for i in [12, 24]], 'gray', width=r*6) # Right torso
            agnostic_draw.line([tuple(pose_data[i]) for i in [23, 24]], 'gray', width=r*12) # Bottom torso
            agnostic_draw.polygon([tuple(pose_data[i]) for i in [11, 12, 23, 24]], 'gray', 'gray')

            # mask neck
            pointx, pointy = pose_data[0]
            ...
            logger.error(f"Error in get_img_agnostic: {str(e)}")
            raise
        ...
        try:
            ...
            for key in self.c_names:
                try:
                    ...
                    logger.error(f"Error loading clothing image {c_name[key]}: {str(e)}")
                    raise
            ...
            try:
                ...
                with open(osp.join(self.data_path, 'mediapipe_json', pose_name), 'r') as f:
                    ...
                    if not pose_label.get('people') or not pose_label['people'][0].get('pose_keypoints_2d'):
                        raise ValueError("No pose data found in the JSON file")
                    pose_data = pose_label['people'][0]['pose_keypoints_2d']
                    pose_data = np.array(pose_data)
                    if pose_data.size == 0:
                        raise ValueError("Pose data is empty")
                    pose_data = pose_data.reshape((-1, 3))[:, :2]
                    pose_data[:,0] *= self.load_width
                    pose_data[:,1] *= self.load_height

            except Exception as e:
                print(f"Error loading pose data for {img_name}: {str(e)}")
                raise
            ...
            try:
                ...
                logger.error(f"Error loading parsing image {parse_name}: {str(e)}")
                raise
            ...
            for i in range(len(labels)):
                for label in labels[i][1]:
                    if label < parse_agnostic_map.size(0):
            ...
            try:
                ...
                img_agnostic = self.transform(img_agnostic)
                    ...
                    logger.error(f"Error processing person image {img_name}: {str(e)}")
                    raise
...
class VITONDataLoader:
    def __init__(self, opt, dataset):
        ...
        try:
            ...
            print(f"Error initializing VITONDataLoader: {str(e)}")
            raise
    ...
    def next_batch(self):
        try:
            ...
            logger.error(f"Error in next_batch: {str(e)}")
            raise
